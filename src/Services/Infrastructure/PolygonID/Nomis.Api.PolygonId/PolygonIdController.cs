...
using Nomis.PolygonId.Interfaces.Requests;
...
namespace Nomis.Api.PolygonId
{
        ...
        ControllerBase
    {
        ...
        /// <param name="mintedScore">Nomis score value.</param>
        /// <param name="expiration">Claim expiration timestamp.</param>
        /// <param name="multichainBlockchains">Blockchains in which the score is calculated.</param>
        /// <param name="cancellationToken"><see cref="CancellationToken"/>.</param>
        /// <response code="200">Returns created claim QR code.</response>
        /// <response code="400">Request data not valid.</response>
        /// <response code="404">No data found.</response>
        /// <response code="500">Unknown internal error.</response>
        [HttpPost("issuer/claims/score/create-with-qr", Name = "PolygonIdCreateScoreClaimWithQr")]
        [SwaggerOperation(
            OperationId = "PolygonIdCreateScoreClaimWithQr",
            Tags = new[] { PolygonIdTag })]
        [ProducesResponseType(typeof(DIDData), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(ErrorResult<string>), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(typeof(ErrorResult<string>), StatusCodes.Status404NotFound)]
        [ProducesResponseType(typeof(RateLimitResult), StatusCodes.Status429TooManyRequests)]
        [ProducesResponseType(typeof(ErrorResult<string>), StatusCodes.Status500InternalServerError)]
        [Produces(MediaTypeNames.Application.Json)]
        public async Task<IActionResult> PolygonIdCreateScoreClaimWithQrAsync(
            WalletPolygonIdRequest request,
            int mintedScore,
            long expiration,
            IList<string> multichainBlockchains,
            CancellationToken cancellationToken)
        {
            var result = await _polygonIdService.CreateClaimAndGetQrAsync<WalletPolygonIdRequest, PolygonIdWalletFakeStats, PolygonIdTransactionIntervalData>(request, mintedScore, new PolygonIdWalletFakeStats(), expiration, 19999999999991, cancellationToken, string.Join(',', multichainBlockchains)).ConfigureAwait(false);
            return Ok(result);
        }
    }
}