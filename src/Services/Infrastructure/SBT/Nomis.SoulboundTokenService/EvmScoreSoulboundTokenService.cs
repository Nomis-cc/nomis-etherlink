...
namespace Nomis.SoulboundTokenService
{
        ...
        IDisposable
    {
            ...
            ScoreSoulboundTokenRequest request)
        {
            ...
            switch (version)
            {
                ...
                case "1.0":
                    ...
                    break;
                case "0.9":
                    var scoreMessageV9 = new SetScoreMessageV9
                    {
                        Deadline = request.Deadline,
                        Nonce = request.Nonce,
                        Score = request.Score,
                        ...
                        ReferralCode = Sha3Keccack.Current.CalculateHash(Encoding.UTF8.GetBytes(request.ReferralCode!)),
                        ReferrerCode = Sha3Keccack.Current.CalculateHash(Encoding.UTF8.GetBytes(request.ReferrerCode!)),
                        DiscountedMintFee = request.DiscountedMintFee ?? 0L
                    };

                    typedData = GetScoreTypedDefinition(request.NFTCommonData, request.ScoreType, request.MintChainId, nameof(SetScoreMessage), typeof(Domain), typeof(SetScoreMessageV9));
                    signature = signer.SignTypedDataV4(scoreMessageV9, typedData, key);
            ...
            ScoreSoulboundTokenImageRequest request)
        {
            ...
            {
                case ImageServiceVersion.V1:
                    string imageV1Request = $"/api/score?address={request.Address}&type={request.Type?.ToLower()}&score={request.Score}&size={request.Size}&chainId={request.ChainId}";
                    response = await _tokenImageClient.GetAsync(imageV1Request).ConfigureAwait(false);
                    break;
                case ImageServiceVersion.V2:
                    string imageV2Request = $"/api/images/sbt/{request.Skin.ToDescriptionString()}?address={request.Address}&score={request.Score}&size={request.Size}&timestamp={request.Timestamp}";
                    if (request.NameServices.Any())
                    {
                        imageV2Request += $"&nameServices={string.Join(',', request.NameServices.Select(x => x.ToDescriptionString()))}";
                    }

                    response = await _tokenImageClientV2.GetAsync(imageV2Request).ConfigureAwait(false);
                    break;
            }
