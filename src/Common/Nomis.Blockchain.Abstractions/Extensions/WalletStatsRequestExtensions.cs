...
namespace Nomis.Blockchain.Abstractions.Extensions
{
    ...
    public static class WalletStatsRequestExtensions
    {
        ...
        /// <summary>
        /// Get wallet stats cache key.
        /// </summary>
        /// <param name="request"><see cref="WalletStatsRequest"/>.</param>
        /// <param name="chainId">Blockchain id.</param>
        /// <returns>Returns wallet stats cache key.</returns>
        public static string GetWalletStatsCacheKey(
            this WalletStatsRequest request,
            ulong chainId)
        {
            return $"{request.Address}_{chainId}_{(int)request.CalculationModel}_{(int)request.ScoreType}_{request.Nonce}_{(int)request.MintBlockchainType}_{(int)request.ScoringChainType}";
        }
